...
class CopyOfTableController {
    ...
    def save() {	
		...
		int postId = session.acountPost.id;
				  ...
				  {//遍历一条记录中的每项数据
					  ...
					  {//更新数据 						 
						  ...
						  if(commit.equals("true")){
							  ...
							  table.receivePost = Post.get(table.inChargePost).parentPost;
						  }
						  ...
						  {//记录中应该无key为id的数据，此处判断多余，需要张验证，如果多余请去除。
							  ...
							  if (aa!=null && aa!="" )
							  {
								...
								//  convert(it,num,tmp);
								  ...
								  def time = PlanTime.findByPostId(postId).planTime;
